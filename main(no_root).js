...
function closedouyin() {
    // 打开应用设置界面
    app.openAppSetting("com.ss.android.ugc.aweme");
    sleep(2000); // 等待2秒，确保设置界面加载完成
    // 查找并点击“结束运行”按钮
    let endButton = className("android.widget.LinearLayout").desc("结束运行").findOne(5000); // 查找按钮，最多等待5秒
    if (endButton && endButton.clickable()) {
        endButton.click(); // 点击“结束运行”
        sleep(1000); // 等待1秒
        // 确认结束运行
        let confirmButton = textMatches(/(.*确.*|.*定.*)/).findOne(5000); // 查找“确定”按钮
        if (confirmButton) {
            confirmButton.click(); // 点击“确定”
        } else {
            log("未找到确认按钮");
        }
        log(app.getAppName("com.ss.android.ugc.aweme") + "应用已被关闭");
        sleep(1000);
        back(); // 返回设置界面
    } else {
        log(app.getAppName("com.ss.android.ugc.aweme") + "应用不能被正常关闭或不在后台运行");
        sleep(1000);
        back(); // 返回设置界面
    }
}
