...
var timeoutId;// 使用settime函数并保存计时器ID

//运行流程
//从本地存储对象中获取密码
friendName = storage.get("passwordd", "");
// 尝试唤醒屏幕
wakeAndUnlock(password).then(() => {//唤醒手机并启动输密码
    return setupUI().then(() => {
    });
});
//从本地存储对象中获取好友名
            ...
            threads.start(function() {
                ...
                if (password) {
                    for (var i = 0; i < password.length; i++) {
                        log("输入密码: " + password[i]);
                        performClick(text(password[i]).untilFindOne());
                    }
                }
            });
            sleep(1000);
...
function sendMessage() {
        ...
        if (currentActivity() != null && packageName === currentActivity()) {
            ...
            if (messagesPageIndicator != null) {
                ...
                var messagesTextWidget = text("消息")
                    ...
                    .untilFindOne();
                performClick(messagesTextWidget);
                sleep(3000, 100);
            } else {
                log("已经在消息页面...");
            }
    ...
    if (text("始终允许").clickable(true).exists()) {
        log("点击始终允许...");
        performClick(text("始终允许").untilFindOne(500));
    } 
    // 如果没有找到“始终允许”，尝试点击“允许”
    else if (text("允许").clickable(true).exists()) {
        log("点击允许...");
        performClick(text("允许").untilFindOne(500));
    }
...
if (typeof friendName === "undefined" || friendName === null) {
    ...
    while (attempts < maxAttempts) {
        ...
        if (!inputBOX.exists()) {
            log("找不到输入框，尝试点击语音按钮");
            let myvoice = desc("语音").findOne(5000);
            if (myvoice) {
                log("找到语音按钮");
                // 调用点击方法
                performClick(myvoice);
                toast("点击语音按钮");
            } else {
            ...
            performClick(inputBOX.findOne(5000));
            sleep(1500);
            setText(messages);
            sleep(100);
            break; // 如果输入框存在，退出循环
...
if (sendmyMessage) {
    ...
    var button = desc("发送").findOne(5000);
    if (button != null) {
        performClick(button);
        toast("发送成功");
        log("发送成功");
...
function closePopup() {
    // 检测并点击常见的关闭按钮
    var btnClose = textMatches(/(.*允许.*|.*以后再说.*|.*不升级.*|.*取消.*|.*跳过.*|.*下次.*)/).findOnce();
    if (btnClose) {
        log("关闭弹窗...");
        performClick(btnClose);
    }

    // 检测并点击特定的控件
    var specificControl = desc("关闭").findOne(1000);
    if (specificControl) {
        log("找到特定控件，点击关闭按钮...");
        performClick(specificControl);
    }
}
// 打开抖音函数
